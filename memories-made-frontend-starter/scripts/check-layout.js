...
(() => {
  ...
  const links = [...nav.querySelectorAll("a, button")].filter(
    (node) => node.getBoundingClientRect().width > 0,
  );
  const ambient = getComputedStyle(
    document.querySelector(".home-shell"),
    "::before",
  );
  const checks = {
    noHorizontalOverflow: document.documentElement.scrollWidth === innerWidth,
    responsiveDestinations: links.length === (innerWidth <= 900 ? 3 : 7),
    touchTargets: links.every(
    ...
    ctaInsideHero:
      rect(".hero__actions").right <= innerWidth &&
      rect(".hero__actions").bottom <= rect(".hero").bottom,
    ctaAndAvailabilityDoNotOverlap:
      rect(".hero__actions").bottom <= rect(".availability-card").top ||
      rect(".hero__actions").right <= rect(".availability-card").left,
    visibleAvailabilityClearsNav:
      availability.top >= innerHeight || availability.bottom <= before.top,
  ...
  checks.navRemainsFixedAfterScroll =
    Math.abs(nav.getBoundingClientRect().top - before.top) < 1;
  checks.footerClearsNav =
    rect(".site-footer").bottom <= nav.getBoundingClientRect().top + 1;
